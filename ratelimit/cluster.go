...
func newClusterRateLimiter(s Settings, sw Swarmer, name string) *clusterLimit {
	...
	go func() {
		for {
			select {
			case size := <-rl.resize:
				log.Debugf("%s resize clusterRatelimit: %v", name, size)
				// TODO(sszuecs): call with "go" ?
				rl.Resize(size.s, rl.maxHits/size.n)
			case <-rl.quit:
				log.Debugf("%s: quit clusterRatelimit", name)
				close(rl.resize)
				return
			}
		}
...
func (c *clusterLimit) Allow(s string) bool {
	...
	if err := c.swarm.ShareValue(key, t0); err != nil {
		log.Errorf("%s clusterRatelimit failed to share value: %v", c.name, err)
	}
	...
	log.Debugf("%s clusterRatelimit: Allow=%v, %v < %d", c.name, result, rate, c.maxHits)
	return result
}
...
func (c *clusterLimit) calcTotalRequestRate(now int64, swarmValues map[string]interface{}) float64 {
	...
	for _, v := range swarmValues {
		...
		log.Debugf("%s: %0.2f += %0.2f / %0.2f", c.name, requestRate, maxNodeHits, adjusted)
		requestRate += maxNodeHits / adjusted
	}
	log.Debugf("%s requestRate: %0.2f", c.name, requestRate)
	return requestRate
}
